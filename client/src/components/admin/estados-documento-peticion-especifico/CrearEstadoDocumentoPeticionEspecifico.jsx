...
const CrearEstadoDocumentoPeticionEspecifico = ({opcionAbierta, cerrarOpcion}) => {
    ...
    const nuevoEstado = useNewMoralisObject("EstadoDocumentosPeticionEspecifico");

    const saveObjetoEstado = async () => {
      nuevoEstado.save(estadoData, {
        onSuccess: estado => {
          console.log("Se creo un nuevo estado documento peticion especifico: " + estado.id)
        },
    ...
    const validarCodigo = () => {
        const codigosExistentes = estadoDocumentoPeticionEspecifico.data.find(codigo => codigo.get("codigo") === estadoData.codigo)
        return (codigosExistentes)
      }

      const validarNombre = () => {
        const nombresExistentes = estadoDocumentoPeticionEspecifico.data.find(nombre => nombre.get("nombre") === estadoData.nombre)
      ...
      const handleSubmit = (e) => {
        ...
        if (e.target.checkValidity() === false) {
          ...
          if (!validarCodigo()) {
            if (!validarNombre()) {
              saveObjetoEstado();
              cerrarOpcion("CrearEstadoDocumentoPeticionEspecifico")
            } else {
  ...
  >
    <Modal
      show={opcionAbierta}
      onHide={() => cerrarOpcion("CrearEstadoDocumentoPeticionEspecifico")}
      centered
    >

      <Modal.Header className="perfil-usuario glassmorphism">
        <Modal.Title className="w-100 title-modal">CREAR ESTADO DOCUMENTO PETICION CONVENIO ESPECIFICO</Modal.Title>
      </Modal.Header>
      ...
      <Modal.Body className="perfil-usuario">
        ...
        >
          ...
          <Button
            ...
            onClick={() => cerrarOpcion("CrearEstadoDocumentoPeticionEspecifico")}
          >Cancelar</Button>
...
export default CrearEstadoDocumentoPeticionEspecifico
