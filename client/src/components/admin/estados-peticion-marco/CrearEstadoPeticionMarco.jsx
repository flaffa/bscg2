...
const CrearEstadoPeticionMarco = ({ opcionAbierta, cerrarOpcion }) => {
  ...
  const nuevoEstado = useNewMoralisObject("EstadoPeticionMarco");

  const saveObjetoEstado = async () => {
    nuevoEstado.save(estadoData, {
      onSuccess: estado => {
        console.log("Se creo un nuevo estado peticion marco: " + estado.id)
      },
  ...
  const validarCodigo = () => {
    const codigosExistentes = estadosPeticionMarco.data.find(codigo => codigo.get("codigo") === estadoData.codigo)
    return (codigosExistentes)
  }

  const validarNombre = () => {
    const nombresExistentes = estadosPeticionMarco.data.find(nombre => nombre.get("nombre") === estadoData.nombre)
  ...
  const handleSubmit = (e) => {
    ...
    if (e.target.checkValidity() === false) {
      ...
      if (!validarCodigo()) {
        if (!validarNombre()) {
          saveObjetoEstado();
          cerrarOpcion("CrearEstadoPeticionMarco")
        } else {
  ...
  return (
    ...
    >
      <Modal
        show={opcionAbierta}
        onHide={() => cerrarOpcion("CrearEstadoPeticionMarco")}
        centered
      >

        <Modal.Header className="perfil-usuario glassmorphism">
          <Modal.Title className="w-100 title-modal">CREAR ESTADO PETICION CONVENIO MARCO</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body className="perfil-usuario">
          ...
          >
            ...
            <Button
              ...
              onClick={() => cerrarOpcion("CrearEstadoPeticionMarco")}
            >Cancelar</Button>
...
export default CrearEstadoPeticionMarco
