...
const CrearEstadoPeticionEspecifico = ({ opcionAbierta, cerrarOpcion }) => {
  ...
  const nuevoEstado = useNewMoralisObject("EstadoPeticionEspecifico");

  const saveObjetoEstado = async () => {
    nuevoEstado.save(estadoData, {
      onSuccess: estado => {
        console.log("Se creo un nuevo estado peticion Especifico: " + estado.id)
      },
  ...
  const validarCodigo = () => {
    const codigosExistentes = estadosPeticionEspecifico.data.find(codigo => codigo.get("codigo") === estadoData.codigo)
    return (codigosExistentes)
  }

  const validarNombre = () => {
    const nombresExistentes = estadosPeticionEspecifico.data.find(nombre => nombre.get("nombre") === estadoData.nombre)
  ...
  const handleSubmit = (e) => {
    ...
    if (e.target.checkValidity() === false) {
      ...
      if (!validarCodigo()) {
        if (!validarNombre()) {
          saveObjetoEstado();
          cerrarOpcion("CrearEstadoPeticionEspecifico")
        } else {
  ...
  return (
    ...
    >
      <Modal
        show={opcionAbierta}
        onHide={() => cerrarOpcion("CrearEstadoPeticionEspecifico")}
        centered
      >

        <Modal.Header className="perfil-usuario glassmorphism">
          <Modal.Title className="w-100 title-modal">CREAR ESTADO PETICION CONVENIO ESPECIFICO</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body className="perfil-usuario">
          ...
          >
            ...
            <Button
              ...
              onClick={() => cerrarOpcion("CrearEstadoPeticionEspecifico")}
            >Cancelar</Button>
...
export default CrearEstadoPeticionEspecifico
