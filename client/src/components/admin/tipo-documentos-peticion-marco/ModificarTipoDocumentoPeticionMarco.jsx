...
const ModificarTipoDocumentoPeticionMarco = ({ opcionAbierta, cerrarOpcion, tipoDocumentoPeticionMarco }) => {
    ...
    const handleChangeTipoDocumentoSeleccionado = (e) => {
        const indexSeleccionado = tipoDocumentoPeticionMarco.findIndex(doc => doc.get("nombre") === e.target.value);
        setTipoDocumentoSeleccionado(prevState => ({
            nombre: e.target.value,
            index: indexSeleccionado,
        }))
        setTipoDocumentoData({
            nombre: tipoDocumentoPeticionMarco[indexSeleccionado].get("nombre"),
            descripcion: tipoDocumentoPeticionMarco[indexSeleccionado].get("descripcion"),
        })
    }
    ...
    const obtenerTipoDocumentoAsociadoPeticionMarco = useMoralisQuery("TipoDocumentoAsociadoPeticionMarco", q => q, [], { live: true });

    const validarNombre = () => {
        const nombresExistentes = obtenerTipoDocumentoAsociadoPeticionMarco.data.find(nombre => nombre.get("nombre") === tipoDocumentoData.nombre && nombre.get("nombre") !== tipoDocumentoSeleccionado.nombre)
    ...
    const handleConfirmSubmit = () => {
        const index = tipoDocumentoPeticionMarco.findIndex( doc => doc.get("nombre") === tipoDocumentoSeleccionado.nombre)
        tipoDocumentoPeticionMarco[index].set("nombre", tipoDocumentoData.nombre);
        tipoDocumentoPeticionMarco[index].set("descripcion", tipoDocumentoData.descripcion);
        tipoDocumentoPeticionMarco[index].save();
        cerrarOpcion("ModificarTipoDocumentoPeticionMarco");
        }
    ...
    return (
      ...
      >
        <Modal
          show={opcionAbierta}
          onHide={() => cerrarOpcion("ModificarTipoDocumentoPeticionMarco")}
          centered
        >
          ...
          {showAlerta
            ...
            : <>
              <Modal.Header className="perfil-usuario glassmorphism">
                <Modal.Title className="w-100 title-modal">MODIFICAR TIPO DOCUMENTO PETICION CONVENIO MARCO</Modal.Title>
              </Modal.Header>
              ...
              <Modal.Body className="perfil-usuario">
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => cerrarOpcion("ModificarTipoDocumentoPeticionMarco")}
                  >Cancelar</Button>
...
export default ModificarTipoDocumentoPeticionMarco
