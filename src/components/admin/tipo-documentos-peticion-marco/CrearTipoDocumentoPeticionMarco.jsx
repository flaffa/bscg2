...
const CrearTipoDocumentoPeticionMarco = ({ opcionAbierta, cerrarOpcion }) => {
    ...
    const nuevoTipoDocumento = useNewMoralisObject("TipoDocumentoAsociadoPeticionMarco");

    const saveObjetoTipoDocumento = async () => {
        nuevoTipoDocumento.save(tipoDocumentoData, {
            onSuccess: tipoDoc => {
                console.log("Se creo un nuevo tipo de documento peticion Marco: " + tipoDoc.id)
            },
    ...
    const validarCodigo = () => {
        const codigosExistentes = tipoDocumentoPeticionMarco.data.find(codigo => codigo.get("codigo") === tipoDocumentoData.codigo)
        return (codigosExistentes)
    }

    const validarNombre = () => {
        const nombresExistentes = tipoDocumentoPeticionMarco.data.find(nombre => nombre.get("nombre") === tipoDocumentoData.nombre)
        ...
        if (e.target.checkValidity() === false) {
            ...
            if (!validarCodigo()) {
                if (!validarNombre()) {
                    ...
                    cerrarOpcion("CrearTipoDocumentoPeticionMarco")
                } else {
    ...
    return (
        ...
        >
            <Modal
                show={opcionAbierta}
                onHide={() => cerrarOpcion("CrearTipoDocumentoPeticionMarco")}
                centered
            >

                <Modal.Header className="perfil-usuario glassmorphism">
                    <Modal.Title className="w-100 title-modal">CREAR TIPO DOCUMENTO PETICION CONVENIO MARCO</Modal.Title>
                </Modal.Header>
                ...
                <Modal.Body className="perfil-usuario">
                    ...
                    >
                        ...
                        <Button
                            ...
                            onClick={() => cerrarOpcion("CrearTipoDocumentoPeticionMarco")}
                        >Cancelar</Button>
...
export default CrearTipoDocumentoPeticionMarco
