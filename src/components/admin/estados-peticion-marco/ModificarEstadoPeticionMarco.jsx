...
const ModificarEstadoPeticionMarco = ({opcionAbierta, cerrarOpcion, estadosPeticionMarco}) => {
  ...
  const handleChangeEstadoSeleccionado = (e) => {
    const indexSeleccionado = estadosPeticionMarco.findIndex( estado => estado.get("nombre") === e.target.value);
    setEstadoSeleccionado( prevState => ({
      nombre: e.target.value,
      index: indexSeleccionado,
    }))
    setEstadoData({
      nombre: estadosPeticionMarco[indexSeleccionado].get("nombre"),
      descripcion: estadosPeticionMarco[indexSeleccionado].get("descripcion"),
    })
  }
  ...
  const handleChangeEstado = (e) => {
    ...
    const estadosPeticionMarcoNombre = useMoralisQuery("EstadoPeticionMarco", q => q, [], { live: true });

    const validarNombre = () => {
      const nombresExistentes = estadosPeticionMarcoNombre.data.find(nombre => nombre.get("nombre") === estadoData.nombre && nombre.get("nombre") !== estadoSeleccionado.nombre)
  ...
  const handleConfirmSubmit = () => {
    const index = estadosPeticionMarco.findIndex( estado => estado.get("nombre") === estadoSeleccionado.nombre)
    estadosPeticionMarco[index].set("nombre", estadoData.nombre);
    estadosPeticionMarco[index].set("descripcion", estadoData.descripcion);
    estadosPeticionMarco[index].save();
    cerrarOpcion("ModificarEstadoPeticionMarco");
    }
  ...
  return(
    ...
    >
      <Modal
        show={opcionAbierta}
        onHide={() => cerrarOpcion("ModificarEstadoPeticionMarco")}
        centered
      >
        ...
        {showAlerta
          ...
          : <>
            <Modal.Header className="perfil-usuario glassmorphism">
              <Modal.Title className="w-100 title-modal">MODIFICAR ESTADO PETICION CONVENIO MARCO</Modal.Title>
            </Modal.Header>
              ...
              >
                ...
                <Form.Group className="mb-3" controlId="estadoAModificar">
                  ...
                  >
                    ...
                    {estadosPeticionMarco.map( (estado,index) =>
                ...
                <Button
                  ...
                  onClick={() => cerrarOpcion("ModificarEstadoPeticionMarco")}
                >Cancelar</Button>
...
export default ModificarEstadoPeticionMarco;
